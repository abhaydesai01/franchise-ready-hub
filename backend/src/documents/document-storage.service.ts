...
export class DocumentStorageService {
  ...
  async savePdfFile(fileName: string, buffer: Buffer): Promise<{ url: string }> {
    ...
    if (!base) {
      this.log.warn(
        'PUBLIC_BASE_URL not set — document URL will be a relative path only',
      );
      return { url: `/uploads/documents/${fileName}` };
    }
    return { url: `${base}/uploads/documents/${fileName}` };
  }
  ...
  async saveDocumentPdf(
    ...
    const safeType = documentType === 'proposal' ? 'Proposal' : 'MOM';
    const fileName = `${safeType}-${leadId}-${Date.now()}.pdf`;
    const bucket = this.config.get<string>('s3.bucket') ?? '';

    return this.savePdfFile(fileName, buffer);
  }
